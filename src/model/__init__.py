...
class Model(nn.Module):
    ...
    def save(self, apath, epoch, is_best=False, lrate=None):
        ...
        if self.save_models:
            save_dirs.append(
                os.path.join(apath, 'model_{}_lrate{}.pt'.format(epoch, lrate))
            )
        for s in save_dirs:
            to_save = {
    ...
    def load(self, apath, pre_train='', resume=-1, cpu=False):
        ...
        if load_from:
            if 'state_dict' in load_from: # @mst: for pruned models, load the pruned model arch as 'self.model'
                if 'model' in load_from and 'arch' not in load_from: # Old interface, to maintain back-compatibility
                    model_ = load_from['model']
                    model_.load_state_dict(load_from['state_dict'], strict=False)
                    self.model = model_.model
                else:
                    self.model = load_from['arch']
                    self.model.load_state_dict(load_from['state_dict'], strict=False)
                print(f'Use the model in ckpt. Weights loaded.')
            else:
